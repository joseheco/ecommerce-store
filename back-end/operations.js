...
function productByCategory(pool, catName, callback) {
  pool.getConnection(function(err, connection) {
    if(err) throw err;
  connection.query(`SELECT * FROM (SELECT product.name, product.url_image, product.price,
                    product.discount, category.name AS catName FROM product INNER JOIN category
                    WHERE product.category = category.id) A WHERE A.catName = "${catName}"`,
                    function(err, result){
      if(err) throw err;
      callback(result);
      connection.release();
